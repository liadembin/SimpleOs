...
// Page directory and tables - statically allocated and aligned to 4KB boundaries
...
void init_paging() {
    ...
    for (uint32_t i = 0; i < 256; i++) {
        // Map physical addresses starting at 0x1000 to virtual addresses starting at 0xC0001000
        kernel_page_table[i] = ((0x1000 + i * 0x1000)) | (PTE_PRESENT_BIT | PTE_READ_WRITE_BIT);
    }

    // Set up page directory entries
    // Entry 0: Identity mapping for the first 1MB
    page_directory[0] = ((uint32_t)first_page_table) | (PDE_PRESENT_BIT | PDE_READ_WRITE_BIT);
    
    // Entry 768 (0xC0000000 >> 22): Higher half kernel mapping
    page_directory[768] = ((uint32_t)kernel_page_table) | (PDE_PRESENT_BIT | PDE_READ_WRITE_BIT);
...
void* map_page(uint32_t physical_addr, uint32_t virtual_addr, uint32_t flags) {
    ...
    if (!(page_directory[pd_index] & PDE_PRESENT_BIT)) {
        ...
        page_directory[pd_index] = ((uint32_t)new_page_table) | (PDE_PRESENT_BIT | PDE_READ_WRITE_BIT);
    }
...
void init_heap(void* start, size_t size) {
    ...
    BlockMetadata* metadata = (BlockMetadata*)((uintptr_t)heap_start + sizeof(DoubleNode));
...
static void split_block(DoubleNode* node, size_t size, uint32_t alignment) {
    ...
    if (remaining_size > sizeof(DoubleNode) + sizeof(BlockMetadata) + sizeof(void*)) {
        ...
        DoubleNode* new_node = (DoubleNode*)((uintptr_t)node->value + size + adjustment);
        BlockMetadata* new_metadata = (BlockMetadata*)((uintptr_t)new_node + sizeof(DoubleNode));
        
        // Set up new metadata
        new_metadata->size = remaining_size - sizeof(DoubleNode) - sizeof(BlockMetadata);
...
static void merge_blocks(DoubleNode* node) {
    ...
    if (node->next != NULL) {
        ...
        if (next_metadata->is_free) {
            ...
            metadata->size += sizeof(DoubleNode) + sizeof(BlockMetadata) + next_metadata->size;
    ...
    if (node->prev != NULL) {
        ...
        if (prev_metadata->is_free) {
            // Calculate combined size
            prev_metadata->size += sizeof(DoubleNode) + sizeof(BlockMetadata) + metadata->size;
